...
class iRPE(nn.Module):
    ...
    def reset_parameters(self):
        ...
        if self.transposed:
            if self.mode == 'bias':
                ...
                self.lookup_table_weight_w = nn.Parameter(
                    torch.zeros(self.num_heads,
                                self.head_dim, self.num_buckets))
                self.lookup_table_weight_b = nn.Parameter(
                    torch.zeros(self.num_heads,
                                self.head_dim, self.num_buckets))
                self.initializer(self.lookup_table_weight_w)
                self.initializer(self.lookup_table_weight_b)
        else:
            if self.mode == 'bias':
                raise NotImplementedError(
    ...
    def forward_rpe_transpose(self, x, rp_bucket):
        ...
        if self.mode == 'bias':
            ...
            """
            lookup_table_weight = self.lookup_table_weight_w*self._ilayer+self.lookup_table_weight_b
    ...
    def forward_rpe_no_transpose(self, x, rp_bucket):
        ...
        lookup_table_weight = self.lookup_table_weight_w*self._ilayer+self.lookup_table_weight_b
        weight = lookup_table_weight[:, rp_bucket.flatten()].\
