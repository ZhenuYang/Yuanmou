...
from engine import train_one_epoch, evaluate, train_one_epoch_distill
...
def main():
    ...
    model = LearngenePool(instances, blk_legth=args.blk_length)
    model.to(device)
    ...
    if args.ls_init:
        ...
        initialize_model_stitching_layer(model, mixup_fn, temp_loader_train, device, mode=args.init_stitch_mode)
    ...
    if args.eval:
        # if args.learngenepool_ckpt == 'instance_snnet':
        # ckpt_path = '/home/shiboyu/code/02-LearngenePool_v2/train_results/finetune_depth9/train_w_distill/instance_ours/49epoch_checkpoint.pth'
        # ckpt_path = '/home/shiboyu/code/02-LearngenePool_v2/train_results/finetune_depth9/train_w_distill/instance_ours/checkpoint.pth'
        # ckpt_path = '/home/shiboyu/code/02-LearngenePool_v2/train_results/finetune_depth6/train_w_distill/instance_snnet/checkpoint.pth'
        ckpt_path = '/home/shiboyu/code/02-LearngenePool_v2/train_results/finetune_depth9/train_w_distill/snnet_snnet/checkpoint.pth'
        logger.info('Loading Parameters of the learngene pool')
        param_dict = torch.load(ckpt_path, map_location='cpu')['model']
        # print(param_dict)
        model.load_state_dict(param_dict)
        evaluate_ours(data_loader_val, model, device, os.path.join(args.output_dir, 'stitches_res.txt'), blk_length=args.blk_length)
        return
    ...
    criterion = StitchDistillationLoss(criterion, teacher_model, args.distillation_type, args.distillation_alpha,
    ...
    if args.resume:
        ...
        if args.resume.startswith('https'):
            ...
            ckpt_path = os.path.join(args.resume, '23epoch_checkpoint.pth')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # evaluate(data_loader_val, model, device, mode='scratch')
        is_best = False
        if epoch in [0, 4, 9, 14, 19, 24, 29, 34, 39, 44, 47, 48, 49]:
